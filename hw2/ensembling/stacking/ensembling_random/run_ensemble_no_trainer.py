...
def main():
    ...
    # Task split creation
    task_split_meta = read_json("./random_task_splits.json")
    ...
    total_train_datasets = {k: dataset["train"].filter(lambda x: x["labels"] in task_to_class_list[k]) for k in task_to_class_list.keys()}
    ...
    task_train_datasets = DatasetDict(
        {k: total_train_datasets[k].select(range(0, int(0.85*len(total_train_datasets[k])))) for k in total_train_datasets.keys()}
    )

    held_out_task_train_datasets = concatenate_datasets([total_train_datasets[k].select(range(int(0.85*len(total_train_datasets[k])), len(total_train_datasets[k]))) for k in total_train_datasets.keys()])
