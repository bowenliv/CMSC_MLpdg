...
class Stacking(nn.Module):
    ...
    def __init__(self, class_number=200):
        ...
        self.norm = nn.LayerNorm(5*512)
        self.fc = nn.Linear(5*512, class_number, bias=True)
...
def main():
    ...
    for step, batch in enumerate(combined_train_dataloader):
        with torch.no_grad():
            ...
            for i in range(len(outputs)):
                ...
                count += 1
    out_file = open("train_t1.json", "w")
    out_file2 = open("train_t2.json", "w")
    json.dump(train_data, out_file)
    json.dump(train_data, out_file2, indent=4)
    ...
    for k in tqdm(K_values, desc="KNN training"):

        knn_model = KNN(n_neighbors=k)
            
        knn_model.fit(train_data[2], train_data[3]) 
            
        # train_pred = knn_model.predict(train_feature)
        val_pred = knn_model.predict(eval_data[2])
        
        
        # evaluation
        val_acc = accuracy_score(eval_data[3], val_pred)
        val_error.append(1 - val_acc)
        val_accuracy.append(val_acc)
        if val_acc > best_acc:
            best_acc = val_acc
            best_k = k
            print(f"best accuracy update: {best_acc}, at K={best_k}")

    logger.info(f"Stacking model knn eval: {best_acc}")

    if args.with_tracking:
        accelerator.log(
            {
                f"Stacking_model_knn_val_accuracy": best_acc,
            }
        )
