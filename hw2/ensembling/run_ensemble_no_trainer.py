...
def ensemble_prediction(test_example, 
    ...
    if aggregation_method == "confidence":
        ...
        # as the final prediction
        min_entrpy = float('inf')
        best_model = None
        for task in task_models:
            task_model = task_models[task]
            outputs = task_model(**test_example)
            outputs = torch.softmax(outputs.logits, dim=1)
            entrpy = -torch.sum(outputs * torch.log(outputs))
            if entrpy < min_entrpy:
                min_entrpy = entrpy
                best_model = task_model
        predictions = best_model(**test_example).logits.argmax(-1)
        return predictions
