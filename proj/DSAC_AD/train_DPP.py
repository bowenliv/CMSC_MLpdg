...
parser.add_argument("--local_rank", type=int, help='rank number for initialize.')
...
class Engine(object):
	...
	def train(self, model, dataloader_train, optimizer, config):
		loss_epoch = 0.
		num_batches = 0
		model.train()
		...
		for data in tqdm(dataloader_train):
			...
			for i in range(config.seq_len):
				fronts.append(fronts_in[i].float().cuda())
				if not config.ignore_sides:
					lefts.append(lefts_in[i].float().cuda())
					rights.append(rights_in[i].float().cuda())
				if not config.ignore_rear:
					rears.append(rears_in[i].float().cuda())
				lidars.append(lidars_in[i].float().cuda())

			# driving labels
			command = data['command'].float().cuda()
			gt_velocity = data['velocity'].float().cuda()
			gt_steer = data['steer'].float().cuda()
			gt_throttle = data['throttle'].float().cuda()
			gt_brake = data['brake'].float().cuda()

			# target point
			target_point = torch.stack(data['target_point'], dim=1).float().cuda()

			pred_wp = model(fronts+lefts+rights+rears, lidars, target_point)

			gt_waypoints = [torch.stack(data['waypoints'][i], dim=1).float().cuda() for i in range(config.seq_len, len(data['waypoints']))]
			gt_waypoints = torch.stack(gt_waypoints, dim=1).float().cuda()
...
def main():
	...
	model = SAC_AD(config).cuda()
	model = torch.nn.parallel.DistributedDataParallel(module=model, find_unused_parameters=True)
	...
	for epoch in range(trainer.cur_epoch, args.epochs):
		trainer.train(model, dataloader_train, optimizer, config)
