...
import sys
sys.path.append('SAC_AD_4vs4_0505/')

from leaderboard.autoagents import autonomous_agent
from SAC_AD_4vs4_0505.model import SAC_AD
from SAC_AD_4vs4_0505.config import GlobalConfig
from SAC_AD_4vs4_0505.data import scale_and_crop_image, lidar_to_histogram_features, transform_2d_points
...
def load_network(state_dict, gpu_num):
	keys = list(state_dict.keys())
	for key in keys:
		if gpu_num < 2 and key.startswith('module'):  # remove 'module.'
			state_dict[key[7:]] = state_dict.pop(key)
		elif gpu_num > 1 and not key.startswith('module'):  # add 'module.'
			state_dict['module.' + key] = state_dict.pop(key)
	return state_dict

class SAC_ADAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.net = SAC_AD(self.config)
		checkpoint = torch.load(os.path.join(path_to_conf_file, 'best_model.pth'))
		self.net.load_state_dict(load_network(checkpoint, 1), strict = True)
		self.net.cuda()
		self.net.eval()
	...
	def run_step(self, input_data, timestamp):
		...
		if(self.step  % 2 == 0 or self.step <= 4):
			...
			self.pred_wp = self.net(self.input_buffer['rgb'] + self.input_buffer['rgb_left'] + \
							   ...
							   self.lidar_processed, target_point)
